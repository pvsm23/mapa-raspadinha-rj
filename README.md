...
- **Início da monetização (Google AdSense), controle granular**: 3 controles independentes dentro do painel de Admin, todos guardados no Firestore em campos/documentos separados de propósito (mudar um NUNCA sobrescreve os outros):
  ...
  - **Passo pendente** (fora do código, só o Paulo pode fazer — criar a conta é uma ação externa): 1) criar conta em [adsense.google.com](https://www.google.com/adsense) com os dados de pagamento; 2) adicionar o site do Desbrava e aguardar a aprovação do Google (pode levar alguns dias); 3) depois de aprovado, criar uma unidade de anúncio e copiar o "client ID" (`ca-pub-...`) e o "slot ID"; 4) substituir `SUBSTITUA_AQUI_PELO_SEU_ID_ADSENSE` (client ID) e `SUBSTITUA_AQUI_PELO_ID_DO_SLOT` (slot ID) no `<ins class="adsbygoogle">` dentro de `index.html`; 5) ligar o toggle certo no painel de Admin quando quiser que os anúncios comecem a aparecer.
