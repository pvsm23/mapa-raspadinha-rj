...
- **Verificação por localização (GPS)**: raspar um município é sempre permitido, mas só conta de verdade (contador, Ranking, Conquistas, região completa) depois que a geolocalização do navegador confirmar que a pessoa está mesmo dentro dele — comparando as coordenadas contra o contorno geográfico real do IBGE (`data/rj-municipios.geojson`, o mesmo arquivo usado pra gerar o mapa), com um teste de "ponto dentro do polígono" (`pontoDentroDoPoligono` em `js/script.js`, ray casting clássico, já lida com buracos). Enquanto não verificado, o município fica **vermelho** no mapa (não verde), e a biblioteca de selos marca o item com ⚠️ e borda vermelha — a imagem raspada continua visível normalmente, só o status muda. No popup, aparece um aviso "⚠️ Raspado, mas ainda não verificado" com um botão "📍 Verificar agora que estou aqui" pra tentar de novo sem precisar raspar outra vez (chama `verificarPresencaNoMunicipio` de novo). Município já verificado uma vez nunca perde essa confirmação (mesmo saindo do local depois). **Importante**: municípios raspados antes dessa funcionalidade existir aparecem como "não verificados" até a pessoa confirmar (não tinham essa informação salva) — é só abrir o popup e clicar em "Verificar agora" estando no local (ou de qualquer lugar, pra fins de teste). Pede permissão de localização ao navegador (`navigator.geolocation`) só no momento de raspar/verificar, nunca de forma contínua.
