...
- **Login com Google + Analytics (configurado)**: `js/firebase-config.js` já tem as chaves reais do projeto Firebase `mapa-raspadinha-rj`. `js/auth.js` inicializa Authentication (login com Google) e Analytics (contagem de acessos). Falta confirmar no [Firebase Console](https://console.firebase.google.com) → Authentication → Sign-in method → Google → **Enable**, senão o login dá erro `auth/operation-not-allowed` ao tentar de verdade. Números de acesso aparecem em Firebase Console → Analytics (ou direto em [analytics.google.com](https://analytics.google.com), propriedade `G-C5SBMCKN4H`) — pode levar algumas horas pra aparecer o primeiro dado.
